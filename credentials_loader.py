...
Reads gitignored `.anthropic-credentials.toml` at the repo root. Supports
tux, foundry, anthropic (direct). Injects resolved credentials into os.environ
so existing code reading ANTHROPIC_AUTH_TOKEN / ANTHROPIC_BASE_URL /
ANTHROPIC_API_KEY continues to work.

Detection order for active_provider = "auto":
  1. Existing process env
  2. Tux daemon reachable
  3. Foundry section (base_url + auth_token)
  4. Anthropic section (api_key)
  5. Windows User registry (legacy)
"""
...
def _apply(base_url, auth_token, api_key):
    if base_url: os.environ["ANTHROPIC_BASE_URL"] = base_url
    if auth_token: os.environ["ANTHROPIC_AUTH_TOKEN"] = auth_token
...
def load_credentials() -> str:
    ...
    for provider in order:
        ...
        if provider == "tux":
            ...
            if _tux_reachable(base_url):
                _apply(base_url, token, None); return "tux"
    ...
    if os.environ.get("ANTHROPIC_AUTH_TOKEN") and os.environ.get("ANTHROPIC_BASE_URL"): return "winreg"
    if os.environ.get("ANTHROPIC_API_KEY"): return "winreg"
    return "none"


def detect_available() -> dict:
    cfg = _read_toml()
    tux_url = (cfg.get("tux") or {}).get("base_url") or "http://127.0.0.1:18080"
